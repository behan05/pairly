...
import { RANDOM_BLOCK_API, BLOCKED_USERS_API, PRIVATE_BLOCK_API } from '@/api/config';
import axios from 'axios';

// const BLOCKED_USERS_API = 'http://localhost:8000/api/blocked';
// const PRIVATE_BLOCK_API = 'http://localhost:8000/api/private-block';
...
export function fetchBlockedUsers() {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.get(`${BLOCKED_USERS_API}/users`, { headers });
...
export function blockUser(formData) {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(`${RANDOM_BLOCK_API}/user`, formData, {
        headers
      });
      if (response.data?.success) {
        dispatch(setBlockingDone());
...
export function unblockUser(blockedPartnerId) {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.delete(`${RANDOM_BLOCK_API}/user`, {
        headers,
 ...
 *            - { success: false } on failure
 */
export function privateBlockUser(formData) {
  return async (dispatch) => {
    dispatch(setIsBlocking());
    ...
    try {
      ...
      const response = await axios.post(`${PRIVATE_BLOCK_API}/user`, formData, {
        headers
      });
      if (response.data?.success) {
        dispatch(setBlockingDone());
 ...
 *            - { success: false } on failure
 */
export function privateUnblockUser(blockedUserId) {
  return async (dispatch) => {
    dispatch(setIsBlocking());
    ...
    try {
      ...
      const response = await axios.delete(`${PRIVATE_BLOCK_API}/user`, {
        headers,
        data: { blockedUserId }
      });
      ...
      if (response.data.success) {
        dispatch(setBlockingDone());
        dispatch(clearBlockedState(blockedUserId));
        return { success: true, message: response.data?.message };
      } else {
        dispatch(setBlockingDone());
        dispatch(setModerationError(`${response.data?.error}`));
