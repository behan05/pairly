...
function PartnerProfileModal({ open, onClose, partner }) {
    ...
    const fullStateName = useMemo(() => {
        return partner?.location?.state
            ? State.getStateByCodeAndCountry(partner.location.state, partner.location.country)
                ?.name
            : ''
    }, [partner]);
    ...
    const partnerInterests = [
        {
            label: 'Interests',
            value: partner?.interests?.length > 0 ? partner.interests.join(', ') : 'No Interests Provided',
        ...
        },
        {
            ...
            value: partner?.preferredChatStyle?.length > 0 ? partner.preferredChatStyle.join(', ') : 'Preferred Chat Style Not Provided',
    ...
    return (
        <Box sx={{ px: isSm ? 2 : 3 }}>
            ...
            <BlurWrapper sx={{ display: open ? 'block' : 'none', overflowY: 'auto', maxHeight: '70vh', width: 'auto' }}>
                ...
                <Stack sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
                    <Stack component={'img'} src={partner?.profileImage || defaultAvatar} alt='Partner Profile Image' aria-label='Partner Profile Image' sx={profileImageCommonStyle} />
                    <GraphicEqIcon fontSize='large' sx={{ color: 'success.main' }} />
                    <Stack component={'img'} src={profileData?.profileImage || defaultAvatar} alt='Partner Profile Image' aria-label='Partner Profile Image' sx={profileImageCommonStyle} />
                </Stack>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, my: 2, flexWrap: 'wrap' }}>
                    <Tooltip title={<StyledText text={`${'View Info'}`} />}>
                        <IconButton onClick={() => handleClick('general')}>
                            <InfoOutlinedIcon fontSize='small' sx={{ color: activeSection === 'general' ? 'success.main' : 'text.secondary' }} />
                        </IconButton>
                    </Tooltip>
                    <Tooltip title={<StyledText text={`${'Interests'}`} />}>
                        <IconButton onClick={() => handleClick('interests')}>
                            <LocalOfferOutlinedIcon fontSize='small' sx={{ color: activeSection === 'interests' ? 'success.main' : 'text.secondary' }} />
                        </IconButton>
                    </Tooltip>
                    <Tooltip title={<StyledText text={`${'Request Private Chat'}`} />}>
                        <IconButton onClick={() => handleClick('privateChat')}>
                            <LockPersonOutlinedIcon fontSize='small' sx={{ color: activeSection === 'privateChat' ? 'success.main' : 'text.secondary' }} />
                        </IconButton>
                    </Tooltip>
                </Box>
                            ...
                            {partnerInterests.map((interests, index) => (
                                <Stack key={index}>
                                    <Stack sx={detailsStyles}>
                                        ...
                                        <Typography variant='body2' color='text.secondary'>{interests.value}</Typography>
                                    </Stack>
                                </Stack>
                            ))}
                        </Box>
                    ...
                    {activeSection === 'privateChat' && (
                        <Box flexDirection={'column'}>
                            ...
                            <Typography variant='body2' color='text.secondary' gutterBottom>
                                {requestingPrivateChat ? 'Requested for a private chat.' : 'Click the button below to send a request for a private chat with your partner.'}
                            </Typography>
                            <Button
                                variant='outlined'
                                color='primary'
